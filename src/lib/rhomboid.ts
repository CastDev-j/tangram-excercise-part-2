...
export class Rhomboid {
  ...
  private getVertices(): THREE.BufferAttribute {
    const shortSide = this.size;
    const longSide = this.size * Math.sqrt(2);
    const mirrorFactor = this.mirrored ? -1 : 1;

    const halfLong = longSide / 2;
    const halfOffset = shortSide / (2 * Math.sqrt(2));

    const vertices = [
      -halfLong - mirrorFactor * halfOffset,
      0,
      -halfOffset,
      halfLong - mirrorFactor * halfOffset,
      0,
